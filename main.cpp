...
int CalculateDaysLeft(const std::wstring& dateStr) {
    ...
    std::tm target = {0};
    target.tm_year = y - 1900; target.tm_mon = m - 1; target.tm_mday = d; target.tm_isdst = -1;
...
std::wstring EncryptString(const std::wstring& input) {
    ...
    if (CryptProtectData(&DataIn, L"MathQuizPwd", NULL, NULL, NULL, 0, &DataOut)) {
        ...
        for (DWORD i = 0; i < DataOut.cbData; ++i) ss << std::hex << std::setw(2) << std::setfill(L'0') << (int)DataOut.pbData[i];
...
std::wstring DecryptString(const std::wstring& hexInput) {
    ...
    for (size_t i = 0; i < hexInput.length(); i += 2) binary.push_back((BYTE)wcstol(hexInput.substr(i, 2).c_str(), NULL, 16));
                ...
                if (localMap.count(id)) {
                    ...
                    if (localItem.lastUpdated > cloudTime) {
                        ...
                        CreateThread(NULL, 0, [](LPVOID p) -> DWORD {
                            ...
                            json j; j["id"] = pData->first; j["is_completed"] = pData->second;
                            SendRequest(L"/api/todos/toggle", "POST", j.dump());
                            delete pData;
                            return 0;
    ...
    if (!countJson.empty()) {
        try {
            ...
            for (const auto& item : j) {
                ...
                g_Countdowns.push_back({item["id"].get<int>(), ToWide(item["title"]), dateW.substr(0, 10), days, cloudTime});
            }
        } catch (...) {}
    }
...
void LoadSettings() {
    WCHAR path[MAX_PATH]; GetModuleFileNameW(NULL, path, MAX_PATH);
    PathRemoveFileSpecW(path); PathAppendW(path, SETTINGS_FILE.c_str());
    g_UserId = GetPrivateProfileIntW(L"Auth", L"UserId", 0, path);
    g_BgAlpha = GetPrivateProfileIntW(L"Settings", L"BgAlpha", 100, path);
    WCHAR buf[256]; GetPrivateProfileStringW(L"Auth", L"Username", L"", buf, 256, path); g_Username = buf;
    GetPrivateProfileStringW(L"Auth", L"Email", L"", buf, 256, path); g_SavedEmail = buf;
    WCHAR pass[1024]; GetPrivateProfileStringW(L"Auth", L"Pass", L"", pass, 1024, path); g_SavedPass = DecryptString(pass);
}
...
void RenderWidget() {
    ...
    bmi.bmiHeader.biWidth = width; bmi.bmiHeader.biHeight = -height; bmi.bmiHeader.biPlanes = 1; bmi.bmiHeader.biBitCount = 32; bmi.bmiHeader.biCompression = BI_RGB;
    void* pBits = nullptr; HBITMAP hBitmap = CreateDIBSection(hdcMem, &bmi, DIB_RGB_COLORS, &pBits, NULL, 0); HBITMAP hOld = (HBITMAP)SelectObject(hdcMem, hBitmap);

    {
        Graphics g(hdcMem); g.SetSmoothingMode(SmoothingModeAntiAlias); g.SetTextRenderingHint(TextRenderingHintClearTypeGridFit);
        ...
        int r = S(15);
        GraphicsPath path; path.AddArc(0, 0, r*2, r*2, 180, 90); path.AddArc(width-r*2, 0, r*2, r*2, 270, 90);
        path.AddArc(width-r*2, height-r*2, r*2, r*2, 0, 90); path.AddArc(0, height-r*2, r*2, r*2, 90, 90); path.CloseFigure();
        SolidBrush bg(Color(g_BgAlpha, 25, 25, 25)); g.FillPath(&bg, &path);

        FontFamily ff(L"MiSans");
        Font titleF(&ff, (REAL)S(16), FontStyleBold, UnitPixel); Font headF(&ff, (REAL)S(12), FontStyleRegular, UnitPixel);
        SolidBrush wBrush(Color(255, 255, 255, 255)); SolidBrush gBrush(Color(255, 180, 180, 180));
        SolidBrush grBrush(Color(255, 80, 220, 80)); SolidBrush rBrush(Color(255, 255, 100, 100));
...
// --- 输入窗口逻辑 ---
HFONT GetMiSansFont(int s) { return CreateFontW(S(s),0,0,0,FW_NORMAL,0,0,0,DEFAULT_CHARSET,0,0,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"MiSans"); }

LRESULT CALLBACK InputWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    if(msg == WM_COMMAND && LOWORD(wp) == IDOK) {
        WCHAR buf[256]; GetDlgItemTextW(hWnd, 101, buf, 256); InputState::result1 = buf;
        if(InputState::currentType == 0) {
            ...
            GetDlgItemTextW(hWnd, 102, buf, 256); InputState::result2 = buf;
        }
        InputState::isOk = !InputState::result1.empty(); DestroyWindow(hWnd); return 0;
    }
...
bool ShowInputDialog(HWND parent, int type, std::wstring& out1, std::wstring& out2) {
    InputState::isOk = false; InputState::currentType = type;
    const wchar_t CN[] = L"InputDlgClass"; UnregisterClassW(CN, g_hInst);
    WNDCLASSW wc={0}; wc.lpfnWndProc=InputWndProc; wc.hInstance=g_hInst; wc.lpszClassName=CN; wc.hbrBackground=(HBRUSH)(COLOR_WINDOW+1); RegisterClassW(&wc);
    HFONT hF = GetMiSansFont(14);
    HWND hDlg = CreateWindowW(CN, (type==0 ? L"添加待办事项" : L"添加倒计时"), WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, S(320), type==0?S(240):S(200), parent, NULL, g_hInst, NULL);

    HWND hL1 = CreateWindowW(L"STATIC", type==0?L"内容:":L"标题:", WS_VISIBLE|WS_CHILD, S(20), S(20), S(50), S(20), hDlg, NULL, g_hInst, NULL); SendMessage(hL1, WM_SETFONT, (WPARAM)hF, 1);
    HWND hE1 = CreateWindowW(L"EDIT", L"", WS_VISIBLE|WS_CHILD|WS_BORDER, S(80), S(20), S(200), S(20), hDlg, (HMENU)101, g_hInst, NULL); SendMessage(hE1, WM_SETFONT, (WPARAM)hF, 1);
    ...
    if(type==0) {
        ...
        HWND hE2 = CreateWindowW(L"EDIT", L"1", WS_VISIBLE|WS_CHILD|WS_BORDER|ES_NUMBER, S(80), S(85), S(200), S(20), hDlg, (HMENU)102, g_hInst, NULL); SendMessage(hE2, WM_SETFONT, (WPARAM)hF, 1);
    } else {
        ...
        HWND hE2 = CreateWindowW(L"EDIT", db, WS_VISIBLE|WS_CHILD|WS_BORDER, S(80), S(50), S(200), S(20), hDlg, (HMENU)102, g_hInst, NULL); SendMessage(hE2, WM_SETFONT, (WPARAM)hF, 1);
    }
    HWND hBtn = CreateWindowW(L"BUTTON", L"确定", WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON, S(110), type==0?S(125):S(100), S(80), S(30), hDlg, (HMENU)IDOK, g_hInst, NULL); SendMessage(hBtn, WM_SETFONT, (WPARAM)hF, 1);

    EnableWindow(parent, 0); MSG msg;
    while(GetMessage(&msg, NULL, 0, 0)) {
        if(msg.message==WM_KEYDOWN && msg.wParam==VK_RETURN) { SendMessage(hDlg, WM_COMMAND, IDOK, 0); continue; }
        TranslateMessage(&msg); DispatchMessage(&msg); if(!IsWindow(hDlg)) break;
    }
    EnableWindow(parent, 1); SetForegroundWindow(parent); DeleteObject(hF);
    out1 = InputState::result1; out2 = InputState::result2; return InputState::isOk;
}

// --- 主窗口过程 ---
LRESULT CALLBACK WidgetWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch(msg) {
    case WM_CREATE: SetTimer(hWnd, 1, 60000, NULL); CreateThread(NULL, 0, [](LPVOID){ SyncData(); return (DWORD)0; }, NULL, 0, NULL); break;
    case WM_TIMER: CreateThread(NULL, 0, [](LPVOID){ SyncData(); return (DWORD)0; }, NULL, 0, NULL); break;
    case WM_USER_REFRESH: RenderWidget(); ResizeWidget(); break;
    case WM_LBUTTONDOWN: {
        int x = LOWORD(lp), y = HIWORD(lp); bool hit = false;
        // 需要加锁读取区域
        std::vector<HitZone> zonesCopy;
        { std::lock_guard<std::recursive_mutex> lock(g_DataMutex); zonesCopy = g_HitZones; } // 使用递归锁

        for(const auto& z : zonesCopy) {
            if(z.rect.Contains(x,y)) {
                hit = true;
                if(z.type == 1) { // Add Todo
                    std::wstring c, d;
                    if(ShowInputDialog(hWnd, 0, c, d)) {
                        // 1. 本地立即添加 (乐观更新) - ID 设为负数作为临时ID
                        {
                            std::lock_guard<std::recursive_mutex> lock(g_DataMutex); // 使用递归锁
                        ...
                        CreateThread(NULL, 0, [](LPVOID p){
                            ...
                            ApiAddTodo(*s);
                            SyncData(); // 上传后立即拉取以获取真实ID
                            delete s; return (DWORD)0;
                        }, new std::wstring(c), 0, NULL);
                    }
                } else if(z.type == 2) { // Add Count
                    std::wstring t, d;
                    if(ShowInputDialog(hWnd, 1, t, d)) {
                        ...
                        CreateThread(NULL, 0, [](LPVOID p){
                            ...
                            SyncData();
                            delete c; return (DWORD)0;
                        }, new Ctx{t,d}, 0, NULL);
                    }
                } else if(z.type == 3) { // Toggle Todo
                    std::lock_guard<std::recursive_mutex> lock(g_DataMutex); // 使用递归锁
                    for(auto& t : g_Todos) {
                        if(t.id == z.id) {
                            t.isDone = !t.isDone;
                            ...
                            bool status = t.isDone; int id = t.id;
                            if (id > 0) { // 只有已同步的（有正ID）才发送toggle，否则依靠AddTodo同步
                                CreateThread(NULL, 0, [](LPVOID p){
                                    auto* pair = (std::pair<int,bool>*)p;
                                    json j; j["id"]=pair->first; j["is_completed"]=pair->second;
                                    SendRequest(L"/api/todos/toggle", "POST", j.dump());
                                    delete pair; return (DWORD)0;
    ...
    case WM_RBUTTONUP: {
        ...
        if(cmd==1001) {
            // 本地先删，提升体验
            {
                std::lock_guard<std::recursive_mutex> lock(g_DataMutex); // 使用递归锁
                if(selType==3) {
                    for(auto it=g_Todos.begin(); it!=g_Todos.end(); ++it) if(it->id==selId) { g_Todos.erase(it); break; }
                } else if(selType==4) {
                    for(auto it=g_Countdowns.begin(); it!=g_Countdowns.end(); ++it) if(it->id==selId) { g_Countdowns.erase(it); break; }
                }
            }
...
// ... Main & Auth functions (LoginWndProc, AttemptAutoLogin, ShowLogin, WinMain) ...
...
LRESULT CALLBACK LoginWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    if(msg==WM_COMMAND && LOWORD(wp)==IDOK) {
        ...
        if(res.empty()) { MessageBoxW(hWnd, L"网络错误", L"Error", 16); return 0; }
        try { auto j = json::parse(res); if(j["success"].get<bool>()) {
            g_UserId = j["user"]["id"]; g_Username = ToWide(j["user"]["username"]);
            SaveSettings(g_UserId, g_Username, e, rem?p:L"", rem);
            g_LoginSuccess = true; DestroyWindow(hWnd);
        } else MessageBoxW(hWnd, L"登录失败", L"Error", 16); } catch(...) {}
    }
...
bool AttemptAutoLogin() {
    ...
    try { auto j = json::parse(res); if(j["success"].get<bool>()) {
        g_UserId = j["user"]["id"]; g_Username = ToWide(j["user"]["username"]);
        SaveSettings(g_UserId, g_Username, g_SavedEmail, g_SavedPass, true); return true;
    }} catch(...) {} return false;
}

bool ShowLogin() {
    const wchar_t CN[] = L"LoginClass"; WNDCLASSW wc={0}; wc.lpfnWndProc=LoginWndProc; wc.hInstance=g_hInst; wc.lpszClassName=CN; wc.hbrBackground=(HBRUSH)(COLOR_WINDOW+1); RegisterClassW(&wc);
    ...
    CreateWindowW(L"STATIC", L"Email:", WS_VISIBLE|WS_CHILD, S(20),S(20),S(60),S(20),h,0,g_hInst,0); g_hEmail=CreateWindowW(L"EDIT", g_SavedEmail.c_str(), WS_VISIBLE|WS_CHILD|WS_BORDER, S(80),S(20),S(180),S(20),h,0,g_hInst,0);
    CreateWindowW(L"STATIC", L"Pass:", WS_VISIBLE|WS_CHILD, S(20),S(50),S(60),S(20),h,0,g_hInst,0); g_hPass=CreateWindowW(L"EDIT", L"", WS_VISIBLE|WS_CHILD|WS_BORDER|ES_PASSWORD, S(80),S(50),S(180),S(20),h,0,g_hInst,0);
    g_hAutoLogin=CreateWindowW(L"BUTTON", L"自动登录 / 记住密码", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, S(80),S(80),S(180),S(20),h,(HMENU)103,g_hInst,0); SendMessage(g_hAutoLogin, BM_SETCHECK, BST_CHECKED, 0);
    ...
    MSG m; while(GetMessage(&m,0,0,0)){ TranslateMessage(&m); DispatchMessage(&m); } DeleteObject(f); return g_LoginSuccess;
}

int APIENTRY WinMain(HINSTANCE hI, HINSTANCE, LPSTR, int nC) {
    g_hInst = hI; SetProcessDPIAware();
    HDC sc = GetDC(0); g_Scale = GetDeviceCaps(sc, LOGPIXELSY)/96.0f; ReleaseDC(0, sc);
    GdiplusStartupInput gsi; ULONG_PTR gt; GdiplusStartup(&gt, &gsi, NULL);
    LoadSettings();
    if(!AttemptAutoLogin()) if(!ShowLogin()) { GdiplusShutdown(gt); return 0; }

    WNDCLASSW wc={0}; wc.lpfnWndProc=WidgetWndProc; wc.hInstance=hI; wc.lpszClassName=L"MathWidget"; wc.hCursor=LoadCursor(0, IDC_ARROW); wc.hbrBackground=(HBRUSH)GetStockObject(BLACK_BRUSH); RegisterClassW(&wc);
    g_hWidgetWnd = CreateWindowExW(WS_EX_LAYERED|WS_EX_TOOLWINDOW, L"MathWidget", L"Math", WS_POPUP, GetSystemMetrics(0)-S(320), S(100), S(300), S(500), 0, 0, hI, 0);
    ShowWindow(g_hWidgetWnd, nC); UpdateWindow(g_hWidgetWnd); ResizeWidget(); RenderWidget();

    MSG m; while(GetMessage(&m,0,0,0)) { TranslateMessage(&m); DispatchMessage(&m); }
    GdiplusShutdown(gt); return (int)m.wParam;
}