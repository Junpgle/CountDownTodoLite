...
#include "utils.h"
#include "api.h"
#include "common.h" // 🚀 必须包含 .h 而非 .cpp，由 common.h 引入全局变量声明
...
// 内部辅助：解析日期字符串为 time_t
time_t ParseWStringDate(const std::wstring &s, bool endOfDay) {
    int y, m, d;
...
float CalculateTodoProgress(const std::wstring &startStr, const std::wstring &endStr) {
    ...
    if (endStr.empty()) {
        ...
        tEnd = ParseWStringDate(startStr, true);
    } else {
        tStart = ParseWStringDate(startStr, false);
        tEnd = ParseWStringDate(startStr, true);
    }
    ...
    HBITMAP hOld = (HBITMAP) SelectObject(hdcMem, hBitmap); {
        ...
        for (const auto &it: g_Countdowns) {
            if (it.daysLeft >= 0) {
                ...
                g.DrawString(dStr.c_str(), -1, &contentF, PointF((REAL) (width - S(65)), y),
        ...
        std::wstring totalStr = (totalSec < 3600)
                                    ? (std::to_wstring(totalSec / 60) + L" m")
                                    : (std::to_wstring(totalSec / 3600) + L" h " + std::to_wstring(
                                           (totalSec % 3600) / 60) + L" m");
        ...
        std::sort(displayList.begin(), displayList.end(), [](const auto &a, const auto &b) {
        ...
        if (displayList.empty()) {
            ...
            for (const auto &rec: displayList) {
                ...
                else if (dn.find(L"phone") != std::wstring::npos || dn.find(L"iphone") != std::wstring::npos || dn.
                         find(L"android") != std::wstring::npos) iconType = 1;
                else iconType = 0;
                ...
                std::wstring tStr = (rec.seconds < 3600)
                                        ? (std::to_wstring(rec.seconds / 60) + L"m")
                                        : (std::to_wstring(rec.seconds / 3600) + L"h");
        ...
        // 过滤掉已完成的待办事项，不显示在主界面
        displayTodos.erase(
            std::remove_if(displayTodos.begin(), displayTodos.end(), [](const auto &t) { return t.isDone; }),
            displayTodos.end());
        ...
        if (displayTodos.empty()) {
            ...
            for (const auto &it: displayTodos) {
                ...
                g.DrawString(dispContent.c_str(), -1, &itemF, PointF((REAL) S(32), y + S(3)),
                             it.isDone ? &gBrush : &wBrush);
                ...
                if (progress >= 0) {
                    ...
                    Color progressColor = (progress > 0.85f && !it.isDone)
                    ...
                    std::wstring dateLabel = (!it.dueDate.empty())
                                                 ? (it.createdDate.substr(0, 10) + L" 至 " + it.dueDate.substr(0, 10))
                                                 : (L"创建于: " + it.createdDate.substr(0, 10));
...
LRESULT CALLBACK WidgetWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
                ...
                if (z.rect.Contains(x, y)) {
                    ...
                    if (z.type == 1) {
                        ...
                        if (ShowInputDialog(hWnd, 0, c, d1, d2)) {
                            ApiAddTodo(c, d1, d2, false);
                            SyncData();
                        }
                    } else if (z.type == 2) {
                        std::wstring t, d, dummy;
                        if (ShowInputDialog(hWnd, 1, t, d, dummy)) {
                            ApiAddCountdown(t, d);
                            SyncData();
                        }
                    } else if (z.type == 3) {
                        if (x < S(30)) {
                            bool done = false; {
                                ...
                                for (auto &t: g_Todos) if (t.id == z.id) done = t.isDone;
                            }
                            ...
                            bool currentDone = false; {
                                std::lock_guard<std::recursive_mutex> l(g_DataMutex);
                                for (auto &t: g_Todos) if (t.id == z.id) {
                                    c = t.content;
                                    d1 = t.createdDate;
                                    d2 = t.dueDate;
                        ...
                        if (MessageBoxW(hWnd, L"确定要删除吗？", L"确认", MB_YESNO) == IDYES) {
                    ...
                    } else if (z.type == 6) { ShowStatsWindow(hWnd); } else if (z.type == 7) {
                        ShowCompletedTodosWindow(hWnd);
                    } // 点击了已完成
                    break;
                }
            }
        ...
        case WM_RBUTTONUP: {
            ...
            // 🚀 计算自定义标签：如果当前值不是预设，则直接在名称里显示数值
            std::wstring customLabel = L"自定义分钟...";
            bool isPreset = (g_SyncInterval == 0 || g_SyncInterval == 5 || g_SyncInterval == 10 || g_SyncInterval == 30
                             || g_SyncInterval == 60);
            ...
            DestroyMenu(hTopSub);
            DestroyMenu(hFreqSub);
            DestroyMenu(hMenu);
        }
        break;
        case WM_USER_REFRESH:
        case WM_USER_TICK: ResizeWidget();
            break;
        case WM_DESTROY: KillTimer(hWnd, SCROLL_TIMER_ID);
            PostQuitMessage(0);
            break;
...
LRESULT CALLBACK InputWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    if (msg == WM_COMMAND && LOWORD(wp) == IDOK) {
        WCHAR content[512];
        GetDlgItemTextW(hWnd, 101, content, 512);
...
bool ShowInputDialog(HWND parent, int type, std::wstring &o1, std::wstring &o2, std::wstring &o3) {
    ...
    if (type != 2) {
        CreateWindowW(L"STATIC", type == 0 ? L"开始日期:" : L"目标日期:", WS_CHILD | WS_VISIBLE, S(20), S(60), S(80), S(20),
                      hDlg, NULL, NULL, NULL);
        HWND hPicker1 = CreateWindowExW(0, DATETIMEPICK_CLASS, L"",
                                        WS_BORDER | WS_CHILD | WS_VISIBLE | DTS_SHORTDATECENTURYFORMAT,
        ...
        if (type == 0) {
            CreateWindowW(L"STATIC", L"截止日期:", WS_CHILD | WS_VISIBLE, S(20), S(100), S(80), S(20), hDlg, NULL, NULL,
                          NULL);
            HWND hPicker2 = CreateWindowExW(0, DATETIMEPICK_CLASS, L"",
                                            WS_BORDER | WS_CHILD | WS_VISIBLE | DTS_SHORTDATECENTURYFORMAT,
    ...
    EnumChildWindows(hDlg, [](HWND h, LPARAM p) {
    ...
    o1 = InputState::result1;
    o2 = InputState::result2;
    o3 = InputState::result3;
...
bool ShowLogin(bool isManualLogout) {
    ...
    CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER, S(100), S(33), S(200), S(25), h, (HMENU)101, NULL,
                  NULL);
    CreateWindowW(L"STATIC", L"密码:", WS_CHILD | WS_VISIBLE, S(40), S(75), S(50), S(20), h, NULL, NULL, NULL);
    CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_PASSWORD, S(100), S(73), S(200), S(25), h,
                  (HMENU)102, NULL, NULL);
    CreateWindowW(L"BUTTON", L"保存密码", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, S(100), S(110), S(90), S(20), h,
                  (HMENU)103, NULL, NULL);
    CreateWindowW(L"BUTTON", L"自动登录", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, S(200), S(110), S(90), S(20), h,
                  (HMENU)104, NULL, NULL);
    CreateWindowW(L"BUTTON", L"登录", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, S(125), S(145), S(100), S(35), h,
                  (HMENU)IDOK, NULL, NULL);
    EnumChildWindows(h, [](HWND ch, LPARAM p) {
        SendMessage(ch, WM_SETFONT, p, TRUE);
        return TRUE;
    }, (LPARAM) hF);
